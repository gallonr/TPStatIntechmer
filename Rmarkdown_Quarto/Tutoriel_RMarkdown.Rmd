...
````markdown
```{r calcul-biomasse-naming-example, eval=FALSE}
...
````markdown
```{r setup-example, include=FALSE, eval=FALSE}
knitr::opts_chunk$set(
  echo = TRUE,
  warning = FALSE,
  message = FALSE,
...
````markdown
```{r calcul-biomasse-example, echo=FALSE, eval=FALSE}
...
````markdown
```{r exemple-formule-noeval, eval=FALSE}
...
````markdown
```{r graphique-croissance-example, fig.cap="Évolution du poids moyen de la daurade en fonction du temps", fig.width=8, fig.height=5, eval=FALSE}
